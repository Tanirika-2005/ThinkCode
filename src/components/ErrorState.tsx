...
export function ErrorState({ onRetry }: ErrorStateProps) {
  return (
    <div 
      ...
      role="alert" 
      aria-live="assertive"
      aria-label="Error loading bus tracking data"
    >
      {/* Enhanced warning icon with better contrast and patterns */}
      <div className="relative w-16 h-16 mb-6 flex items-center justify-center">
        ...
        <div className="absolute inset-2 border-2 border-dashed border-orange-400 rounded-full opacity-50" aria-hidden="true"></div>
      </div>
      
      {/* Error message with enhanced typography */}
      <h2 className="text-xl text-gray-900 text-center mb-3 max-w-sm font-semibold flex items-center gap-2">
        <span className="w-3 h-3 bg-red-600 rounded-full flex-shrink-0" aria-hidden="true"></span>
        Unable to Load Bus Tracking
      </h2>
      
      {/* Sub-message with better contrast */}
      ...
      {/* Enhanced retry button with accessibility features */}
      <button
        onClick={onRetry}
        className="
          w-36 h-12 
      ...
      {/* Additional help text with better contrast */}
      ...
      {/* Screen reader detailed error information */}
      <div className="sr-only">
        Error details: The bus tracking system could not connect to real-time transit data. 
        This may be due to internet connectivity issues or temporary service disruption. 
